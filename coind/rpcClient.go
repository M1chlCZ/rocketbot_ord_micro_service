...
func newClient(host string, port int, wallet, user, passwd string, useSSL bool, timeout int) (c *rpcClient, err error) {
	...
	addr := fmt.Sprintf("%s%s:%d%s", serverAddr, host, port, wallet)
	c = &rpcClient{serverAddr: addr, user: user, passwd: passwd, httpClient: httpClient, timeout: timeout}
	return
}

//http://localhost:8332/wallet/WALLETNAME
//http://127.0.0.1:12300/wallet/ord
...
func (c *rpcClient) call(method string, params any) (rr rpcResponse, err error) {
	...
	if resp.StatusCode != 200 {
		utils.WrapErrorLog(fmt.Sprintf("RPC response: %s", string(data)))
		utils.WrapErrorLog(fmt.Sprintf("RPC response: %d", resp.StatusCode))
		_ = resp.Body.Close()
		_ = req.Body.Close()
		return rpcResponse{}, errors.New(string(data))
	}
